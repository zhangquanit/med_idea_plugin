...
//            GitVcs.getInstance(project).showMessages(result);
    ...
    public String diff() throws VcsException {
        gitWriteLock.lock();
        try {
            String cmd = DIFF_CMD;
...
//    }
    ...
    private String execute(@NotNull String cmd, List<String> cmdArgs, boolean silent) throws VcsException {
        ...
        LogUtil.d("cmdLine=" + StringUtil.join(cmdLine, " "));
...
//        }
        ...
        try {
            ...
            try {
                ...
                LogUtil.d("git execute 失败11 msg=" + ie.getMessage());
                return EMPTY_STRING;
            }
            in.close();
            ...
            GitVcs.getInstance(project).showMessages("执行结果：\n" + output);

            if (cmd.equals(DIFF_CMD) && output.contains("No HEAD commit to compare with")) {
                LogUtil.d("No HEAD commit to compare with");
                return EMPTY_STRING;
            }
            ...
            LogUtil.d("git execute 失败22 msg=" + e.getMessage());
...
//        return VfsUtil.virtualToIoFile(vcsRoot);
        return GitUtil.getDirectory(project);
    }
}
